 ...
 */
function meego_theme(&$existing, $type, $theme, $path) {
...
function meego_preprocess(&$vars, $hook) {
 ...
 */
function meego_preprocess_page(&$vars, $hook) {
...
function meego_preprocess_node(&$vars, $hook) {
	$node = $vars['node'];
	$taxonomy = _meego_sort_node_taxonomy($node); // our own utility function
...
function meego_preprocess_comment_wrapper(&$vars, $hook) {
	$node = $vars['node'];
 ...
 */
function _meego_sort_node_taxonomy($node) {
  $taxonomy = array();
...
function meego_preprocess_comment(&$vars, $hook) {
 ...
 */
function meego_preprocess_block(&$vars, $hook) {
	$block = $vars['block'];
 ...
 */
function meego_username($object) {
 ...
 * Custom Marker
 */
function meego_mark($type = MARK_NEW) {
  global $user;
  if ($user->uid) {
    if ($type == MARK_NEW) {
 ...
 * Custom RSS Icon
 */
function meego_feed_icon($url, $title) {
...
*/
function meego_user_picture($account, $size = 'user_pics') {
 ...
 */
function meego_breadcrumb($breadcrumb) {
 ...
 */
function meego_menu_local_tasks() {
  $output = '';
...
*
*/
function meego_menu_item_link($link) {
 ...
 */
function meego_preprocess_views_view_fields(&$vars) {
  foreach ($vars['fields'] as $field) {
    if ($class = _meego_get_views_field_class($field->handler)) {
      $field->class = $class;
    }
  }
 ...
 */
function meego_preprocess_views_view_table(&$vars) {
  $view = $vars['view'];
  foreach ($view->field as $field => $handler) {
    if (isset($vars['fields'][$field]) && $class = _meego_get_views_field_class($handler)) {
 ...
 */
function _meego_get_views_field_class($handler) {
